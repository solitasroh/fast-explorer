...
- M6-deferred tech-debt: extract IconCacheCoordinator from MainWindow (removes ~60 LOC + collapses the redraw tail). ResultChannel<T> template extraction CLOSED 2026-05-16 (commit 558d895) — IconProvider + ShellWorker share one canonical publish/drain/coalesce path. LowMemoryCallback signature change CLOSED 2026-05-16 — switched to std::function<void()>, removing the static HWND global and the trampoline free function from main-window.cpp.
