...
- **UI stall single ≤ 50 ms** 100k 시나리오 검증 — StallHistogram (`ui/stall-probe.{h,cpp}`) added 2026-05-17: 7-bucket aggregator (<1ms / 1-5ms / 5-16ms / 16-50ms / 50-100ms / 100-500ms / ≥500ms) wired into `runMessageLoop` between Translate+Dispatch ticks; `maxGapMicros` is the single-stall worst case. Dumped to the logger on app shutdown alongside the PerfTracker stream so the 100k scenario stall distribution is captured by any session that loaded a large folder. Quantitative 100k full-UI run (load + scroll-through + close, with the histogram dump captured) remains a follow-up requiring either UI automation or a manual scripted session.
- **scroll p95 ≤ 16.7 ms** 측정 — LVN_GETDISPINFO p99 ≤ 50 µs is the per-row component; DispInfoHistogram (`ui/dispinfo-histogram.{h,cpp}`) added 2026-05-17 brackets `handleGetDispInfo` with QPC and buckets per-call latencies into <1us / 1-5us / 5-20us / 20-50us / 50-100us / 100-500us / ≥500us. The 50us boundary aligns with the §14.3 gate so a p99 estimate ≤ 50 µs is bucket-readable. Dumped to the logger on app shutdown alongside StallHistogram. Frame-level p95 (full scroll-through soak on a RAM disk) remains a follow-up requiring UI automation.
