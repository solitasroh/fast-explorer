...
src/ui/adapters/   638 lines (7 .h + .cpp pairs)

examples/minimal-window/   ~460 lines (demo + in-memory port impls)
```

`src/ui/main-window.cpp` is still ~3,155 lines (~3% smaller than
...
- `FastExplorer`          — shell app, now consumes `winui_lite` through 7 adapters under `src/ui/adapters/`.
...
- WinUI / XAML migration path: replace `chrome/` while keeping the port surface intact; adapters under `src/ui/adapters/` continue to work.
...
- No source-tree rename (`src/ui/` stayed; the plan mentions `src/explorer/` — left for a future cleanup pass).
